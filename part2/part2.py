...
def generate_random_betas_vector():
    betas_vector = []
    for beta in range(number_of_betas):
        random_number = random.randint(0, 2**bits_per_beta)
        betas_vector.append(random_number)
    return betas_vector


def calc_MSE(x, y, beta_vector, num_rows):
...
def get_neighbor(current_betas_vector):
    ...
    '''
    iteration = 0
    for old_beta in current_betas_vector:
